...
public class Circle extends View{
	...
	private final float[] originalColorWeight = {0, 0.33f, 0.66f, 1.0f};//一定不能有0.25、。。把他改成0.245
	...
	public Circle(Context context, float rr){
		...
		y=rr;//圆心
	    wr = rr;//半径 外圈
	    paintWidth = wr * 2 / 366 * 5;//画圆的时候的笔的宽度
	    x = wr+paintWidth;//预留了一个width的长度
	    y = wr+paintWidth;
		r = wr - paintWidth/2;
	...
	private void drawText(Canvas canvas){
		...
		canvas.drawText("分", x - textSize * 20 / 40 + textSize, y + textSize / 4, paint);//看图调节出来的参数
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension( (int) (wr+wr+paintWidth*2 + 0.5f), (int)(wr+wr+paintWidth*2 + 0.5f));//预留一点长度。
	}
